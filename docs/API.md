...
| Method | Endpoint | Keterangan |
|--------|----------|------------|
| POST | `/chatbot` | tanya AI assistant |
| DELETE | `/chatbot/clear` | hapus riwayat chat |

**POST /chatbot** — AI-first: pertanyaan langsung diteruskan ke OpenAI dengan konteks data kos real-time dan riwayat percakapan.
Riwayat disimpan ke `chatbot_history`. Termasuk proteksi prompt injection, SQL injection, dan data leakage.
...
> default `gpt-4o-mini`). Tanpa key, `/chatbot` mengembalikan pesan fallback yang sopan.
